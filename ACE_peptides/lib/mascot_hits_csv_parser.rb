...
class MascotHitsCSVParser
	attr_accessor :filename, :cutoff, :is_ascending, :sign, :id

	def initialize(filename, cutoff, is_ascending, sign, id)
		@filename = filename
		@cutoff = cutoff.to_f
	...
	def self.open(filename, cutoff, is_ascending, sign, id)
		csvp = MascotHitsCSVParser.new(filename,cutoff, is_ascending, sign, id)
		if block_given?
			csvp.each do |hit|
				yield hit
			end
	...
	def create_pep_index()
		...
		@filehandle.each do |line|
			...
			if in_protein_hits_table == true
				if (hit.pep_var_mod.include? 'Acetyl') && (hit.pep_rank.to_i == 1) && (hit.pep_expect < @cutoff)
